...
# 已內置 Chromium + 所有系統依賴，WhatsApp/破 Cloudflare 嘅 adapter 唔使自己裝。
...
ENV PORT=3000
# 持久化資料（SQLite DB + WhatsApp session）走呢個路徑，記得掛 volume
ENV DB_PATH=/app/data/beyhunter.db
ENV WWEBJS_AUTH_PATH=/app/data/wwebjs_auth
# notify-settings.json（WhatsApp 收件人設定）都放 volume，rebuild 唔會冇
